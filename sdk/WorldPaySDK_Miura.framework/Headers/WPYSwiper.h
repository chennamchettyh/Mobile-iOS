 ...
 * @param terminalSelectionsApplication If this is set to true, then application selection will immediately happen if necessary after reading the card, handled by the terminal. If set to false, application selection will be handled by the SDK, which can optionally be overridden with a delegate method didRequestSelectEMVApplication in WPYSwiperDelegate.
 * @param commonDebitMode This uses the USCommonDebitMode enum, which allows values for default multi-application identifier selection, prefer US Common Debit application identifiers, or prefer Global Debit application identifiers. This value is ignored if terminalSelectsApplication is false.
 */
